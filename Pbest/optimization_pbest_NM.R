...
n_treats_start <- 4 #number of treatments
...
simulation <- function(n_treats,
  ...
  return ranks;
      ...
      pbest_calc <- function(x){ #create pbest function that calculates the proportion of times
        sum(x==1)/nsamps #each treatment was ranked first
      }
      
      pbest <- t(as.matrix(apply(t(ranks),1,pbest_calc))); colnames(pbest) <- treats
      #apply function to ranked dataset
      ...
      for(i in 1:n_treats){
        ...
        if(ntot >= min_n){
          if(ind.state == 0){
            pbest_ <- as.numeric(pbest[,treats[i]])
            
            if(pbest_ > thresh_sup){
              Result["Sample.size", treats[i]] <- ntot
              Result["Result", treats[i]] <- "Superiority"
              ind[,treats[i]] = 1
            }
            
            if(pbest_ < thresh_fut){
              Result["Sample.size", treats[i]] <- ntot
              Result["Result", treats[i]] <- "Inferiority"
              ind[,treats[i]] = 2
            }
            
            if(pbest_ <= thresh_sup && pbest_ >= thresh_fut){
              Result["Sample.size", treats[i]] <- ntot
...
#ess<-mean(as.numeric(unlist(results[nums,])))
...
p<-c(0.97, 0.02)
out <- Nelder_Mead(sim_fun, p, lower=c(0,0), upper=c(1,1), control=list(maxfun=100))
