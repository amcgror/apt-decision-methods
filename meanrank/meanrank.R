...
tsup <-c(6,7,8)
T1 <-c(rep(0.05,3))
thresh1 <- c(rep(1.047233887,3))
thresh2 <- c(rep(2.944931641,3))
pow <- c(rep(0, 3))
ess <- c(rep(0,3))
...
simulation <- function(n_treats,
  ...
  return ranks;
      ...
      for(i in 1:n_treats){
        ...
        if(ntot >= min_n){
          if(ind.state == 0){
            meanrank <- as.numeric(mean_rank[,treats[i]])
            
            if(meanrank < thresh_sup){
              Result["Sample.size", treats[i]] <- ntot
              Result["Result", treats[i]] <- "Superiority"
              ind[,treats[i]] = 1
            }
            
            if(meanrank > thresh_fut){
              Result["Sample.size", treats[i]] <- ntot
              Result["Result", treats[i]] <- "Inferiority"
              ind[,treats[i]] = 2
            }
            
            if(meanrank >= thresh_sup && meanrank <= thresh_fut){
              Result["Sample.size", treats[i]] <- ntot
